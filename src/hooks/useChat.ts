...
export const useChat = (options: UseChatOptions) => {
    ...
    const messages = allMessages.length > displayedMessageCount
    ...
    const createAIMessage = useCallback((content: string, metadata?: Message['metadata']): Message => ({
    ...
    const handleSendMessage = useCallback(async () => {
        if (!inputValue.trim() || !challengeAssignmentId || !token || isTyping) return
    ...
    }, [inputValue, challengeAssignmentId, sessionId, token, isTyping, createUserMessage, createAIMessage, onError])
    ...
    const loadMoreMessages = useCallback(() => {
        ...
        setDisplayedMessageCount(prev => Math.min(prev + MESSAGES_PER_LOAD, allMessages.length))
    }, [hasMoreMessages, allMessages.length])

    // Load message history when challenge assignment is available
    useEffect(() => {
        const loadMessageHistory = async () => {
            if (!challengeAssignmentId || !sessionId || !token || hasLoadedHistory.current) {
                return
            }
